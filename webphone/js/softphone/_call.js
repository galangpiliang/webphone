...
//OPSSTART
                        common.Glbr() === true && 
//OPSEND
                        !common.isNull(ratinguri) && ratinguri.length > 2 && !common.isNull(callnumber) && callnumber.length > 0 &&
...
//--        }
    ...
    if (calltype === "incoming")
    {
        if (common.GetParameterBool('autoaccept', false) === true)
        {
            j$('#hangup_layout').show();
            j$('#callfunctions_layout').show();
            j$('#acceptreject_layout').hide();
...
//alert('j$("#hangup_layout").height(): ' + j$("#hangup_layout").height() + '\nj$("#callfunctions_layout").height(): ' + j$("#callfunctions_layout").height() + '\nmax_vid_height: ' + max_vid_height);
            
            if (vh > max_vid_height)
            {
                j$('#video_container').height(max_vid_height);
                j$('#video_remote').height(max_vid_height - 55);
            }
        }
    }
...
// calculate video priority
    ...
    if (global.isdebugversion === true) {for (var i = 0; i < usage.length; i++) { common.PutToDebugLog(5, "cfusage " + usageNames[i] + ": " + usage[i]); }}
...
// if is multiline, then try to find another active call and connect them whitout asking for number
        ...
        for (var i = 0; i < global.ep.length; i++)
        {
            ...
            if (ln !== global.aline) // we found another active call
            {
                ...
                if (!common.isNull(nr) && nr.length > 0)
                {
                    ...
                    {
                        common.ShowToast(stringres.get('interconnect_conf'));
    ...
    if (global.acallcount > 1 && (common.getuseengine() === global.ENGINE_SERVICE || common.getuseengine() === global.ENGINE_WEBPHONE || common.IsWindowsSoftphone() === true))
    {
        interconnectHtml = '<br />'+
            '<button id="btn_interconnect_conf" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" style="font-size: .9em">' + stringres.get('interconnect') + '</button>';
    }
...
//--    j$.mobile.activePage.append(template).trigger("pagecreate");
    ...
    j$.mobile.activePage.find(".messagePopup").popup().popup("open").bind(
    {
        popupafterclose: function ()
        {
            ...
            j$('#adialog_negative').off('click');
            j$('#btn_pickct').off('click');
            j$('#btn_interconnect_conf').off('click');
            popupafterclose();
        }
    });
...
//--		if clicked again, don't initiate conf call again
    ...
    j$('#btn_interconnect_conf').on('click', function (event)
    {
        common.PutToDebugLog(2, 'EVENT, conference interconnect on click')
...
//--    }
    ...
    {
        interconnectHtml = '<br />'+
            '<button id="btn_interconnect_transfer" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" style="font-size: .9em">' + stringres.get('interconnect') + '</button>';
    }
...
//--    j$.mobile.activePage.append(template).trigger("pagecreate");
    ...
    j$.mobile.activePage.find(".messagePopup").popup().popup("open").bind(
    {
        popupafterclose: function ()
        {
            ...
            j$('#adialog_negative').off('click');
            j$('#btn_pickct').off('click');
            j$('#btn_interconnect_transfer').off('click');
            popupafterclose();
        }
    });
...
//--    });
    ...
    j$('#btn_interconnect_transfer').on('click', function (event)
    {
        common.PutToDebugLog(2, 'EVENT, transfer interconnect on click')
...
//--    boolean muteSuccess = sipStack.API_Mute(-1, phoneService.muteState);
    ...
    if (muteDirection === 5)
    {
        common.PutToDebugLog(2, 'WARNING, CALL, API_MutEx, ' + mstate + ', ' + muteDirection + ' Mute is disabled!');
        return false;
    }
