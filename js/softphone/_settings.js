...
//OPSSTART
            && common.Glv() > 1
//OPSEND
            && common.GetParameter('androidchromedisplayed') !== 'true'
            )
        {
...
// we have to wait for File class to load
    if (waitmaxloop > 18)
    {
        common.PutToDebugLog(1, 'ERROR, Failed to start');
        common.PutToDebugLog(2, 'ERROR, _settings GetAccounts File class failed to load');
    }
...
//--###MZSETT                common.InitializeSettings();
//--DBG                 common.PutToDebugLog(3, 'DEBUG, _settings GetAccounts callback no content_2');
//--###MZSETT                common.GetSettingsFromUrl();
//--###MZSETT                common.GetOverWriteSettings();
...
var settOrderWebphoneWebRTCFlash = 'serveraddress_user,sipusername,password,submenu_sipsettings,submenu_media,submenu_calldivert,submenu_general,theme,language,'+ exregacc +'email,incomingcallpopup,sendchatonenter,keeprecfiles,proxyaddress,realm,username,displayname,hidemyidentity,' + utops + 'ringtimeout,calltimeout,enableblf,dtmfmode,chatsms,earlymedia,defmute,customsipheader,normalizenumber,techprefix,filters,voicemailnum,callbacknumber,callforwardonbusy,callforwardonnoanswer,callforwardonnoanswertimeout,callforwardalways,calltransferalways,transfwithreplace,autoignore,autoaccept,dropsameoldcall,blacklist,holdtype,autohold,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,audiodevices,displayvolumecontrols,displayaudiodevice,savetocontacts,telsearchkey,extraoption,pushnotifications,reset_settings,loglevel,loglevel_dbg,codec,alwaysallowlowcodec,icetimeout,'+
        'audio_bandwidth,video,video_bandwidth,video_width,video_height,'+
        'cfgvideo,video_profile,videocodec,'+ aspeak +
//--        'submenu_screenshare,sscontrol,ssscroll,sstop,ssquality,'+
...
var basicSettings = 'settobasefunctionality,theme,username,displayname,' + utops + 'transport,proxyaddress,register,ringtone,audiodevices,startonboot,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,cpualwayspartiallock,forcewifi,email,loglevel,startwithos';
...
//--                 !!!! HACK !!!
                //if (common.GetParameter('showserverinput') === '1' && common.GetConfigInt('upperserverfromuser', 2) < 2)
                if (!common.RequestUserServerInput())
                {
                    continue;
                }
...
// treat the followings as seconds instead of milliseconds if the value is between 2 and 999: ringtimeout,calltimeout,icetimeout,keepaliveival,wskeepaliveival,closecall_timeout
            ...
            {
                var intval = common.StrToInt(mSettValue, -10000);
...
// handle username / sipusername
        ...
        j$('#adialog_positive').on('click', function (event)
        {
            ...
            {
                textBoxVal = common.Trim(textBoxVal);
                ...
                {
                    var intval = common.StrToInt(textBoxVal, -10000);
                    if (intval >= 2 && intval <= 999)
                    {
                        intval = intval * 1000;
                    }
    ...
    }else
    {
        ...
        else if (type === '8')
        {
            ...
            else if (settname === 'serveraddress_user')
            {
                if (common.GetParameter('serverinputisupperserver') === 'true'
//OPSSTART
                    && (common.GetParameter('autoprovisioning').length < 1 || common.GetParameter('autoprovisioning') === '0')
//OPSEND
                    )
                {
...
function IsUsernameFixed()
{
    ...
    } catch(err) { common.PutToDebugLogException(2, "_settings: IsUsernameFixed", err); }
    return false;
}
...
function IsPasswordFixed()
{
    ...
    } catch(err) { common.PutToDebugLogException(2, "_settings: IsPasswordFixed", err); }
    return false;
}
